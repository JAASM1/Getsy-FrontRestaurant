...
const TotalDailyBookings = () => {
  ...
  const getTodayReservations = (reservations: ReservacionProps[]) => {
  ...
  const getReservationCounts = (reservations: ReservacionProps[]) => {
    const todayReservations = getTodayReservations(reservations);

    return {
      cancelled: todayReservations.filter((res) => res.status === "cancelled")
        .length,
      accepted: todayReservations.filter((res) => res.status === "confirmed")
        .length,
      pending: todayReservations.filter((res) => res.status === "pending")
        .length,
    };
  };
  ...
  return (
    <div className="flex flex-col justify-center items-center px-6 md:px-[3rem] w-full font-Poppins">
      <div className="flex w-full justify-between md:justify-normal items-center mb-3 space-x-8">
        ...
        <h1 className="text-xl md:text-2xl font-bold">
          ...
          <span className="font-normal underline ml-2 max-md:hidden">{today}</span>
        </h1>
      </div>
      <div className="w-full md:flex items-center md:px-[4.5rem] mb-5">
        <div className="w-full md:flex justify-between items-end max-md:space-y-1">
          <div className="flex justify-between md:space-x-3 text-sm">
            <div className="flex space-x-1">
              <p>Aceptadas:</p>
              <b>{reservationCounts.accepted}</b>
            </div>
            <div className="flex space-x-1">
              <p>Pendientes:</p>
              <b>{reservationCounts.pending}</b>
            </div>
            <div className="flex space-x-1">
              <p>Canceladas:</p>
              <b>{reservationCounts.cancelled}</b>
            </div>
          </div>
...
export default TotalDailyBookings;
